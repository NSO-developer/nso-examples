...
code to raise an alarm whenever it receives an SNMPv3 notification protected
with `authPriv`, SHA authentication, and AES privacy.
...
managed device sends an SNMPv3 `authPriv` notification:
...
The `sendnotif.sh` script uses the Net-SNMP `snmptrap` application to generate
and send an SNMPv3 `authPriv` trap. The sent trap will be of type
`IF-MIB::linkDown`.
...
notifications directly. Examples:
...
The existing alarm now includes additional status changes with the "test alarm"
alarm text.
