...
NODE_IPC = NCS_IPC_PATH=$(NCS_IPC_PATH).456
endif
endif
...
nso-node%: cdb-init/init.xml packages
	...
	@sed -i.bak \
		...
		-e 's/4569/456$*/g' \
		-e 's/u@ncs/u@n$*/g' \
		-e '/<webui>/,/<\/webui>/ s|<match-host-name>.*</match-host-name>|<match-host-name>false</match-host-name>|' \
		$@/ncs.conf
	if [ -e packages ]; then \
...
start-node%:
	(cd nso-node$* && . ./run.env && $(NODE_IPC)$* ncs)
