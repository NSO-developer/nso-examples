...
env $NODE3 ncs_cli -n -u admin -C << EOF
show high-availability status | notab | nomore
show running-config dummies | nomore
EOF
...
printf "\n"
while [[ $(env $NODE3 ncs_cmd -o -c 'mrtrans; mexists "/high-availability/status/be-secondary-result"') != "yes" ]]; do
...
printf "\n\n"
while [[ $(env $NODE2 ncs_cmd -o -c 'mrtrans; maapi_get "/high-availability/status/mode"') != "secondary" ]] || \
      [[ $(env $NODE3 ncs_cmd -o -c 'mrtrans; mexists "/high-availability/status/be-secondary-result"') != "yes" ]]; do
