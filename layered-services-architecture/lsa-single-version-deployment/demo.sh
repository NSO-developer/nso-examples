...
printf "\n\n${PURPLE}##### Configure the cfs-vlan service\n${NC}"
env ${ENV}4569 ncs_cli -n -u admin -C << EOF
config
...
printf "\n${PURPLE}##### Waiting for upper-nso commit queue items to complete...\n${NC}"
while : ; do
    arr1=($(echo "show ncs:devices commit-queue queue-item | icount" | env ${ENV}4569 ncs_cli -C -u admin))
    res1=${arr1[1]}
    if [ "$res1" == "0" ]; then
        printf "${GREEN}##### All commit queue items completed\n${NC}"
        break
    fi
    printf "${RED}##### Waiting for $res1 upper-nso commit queue items to complete...\n${NC}"
    sleep .1
done
