...
def resolve_odn_color(root, odn_name: str, device_name: str) -> int:
    if odn_name not in root.core_network.services.odn_template:
        raise ValueError(f'Unknown ODN template {odn_name}')

    odn = root.core_network.services.odn_template[odn_name]
    if device_name not in [str(d) for d in odn.head_end]:
        raise ValueError(
            f'ODN template {odn_name} is not provisioned on head-end '
            f'{device_name}'
        )
    return int(odn.color)
...
class ServiceCallbacks(Service):
    ...
    def cb_create(self, tctx, root, service, proplist):
        ...
        with maybe_rm_service(service) as rm_service:
            for link in service.link:
                ...
                params.add(
                    'SRV6',
                    'true'
                    if enum_text(service.transport_profile, 'srv6') == 'srv6'
                    else ''
                )
                ...
                if service.odn_template and (
                    is_xr_device(root, str(port.device)) or
                    is_junos_device(root, str(port.device))
                ):
                    pe_name = str(port.device)
                    if pe_name not in steered_devices:
                        params.add(
                            'ODN_COLOR',
                            resolve_odn_color(root, service.odn_template,
                                              pe_name)
                        )
                        template.apply('l3vpn-odn', params)
                        steered_devices.add(pe_name)
        ...
        if service.odn_template and not steered_devices:
            raise ValueError(
                f'L3VPN {service.name} references ODN template '
                f'{service.odn_template}, but no IOS XR or Junos PE link '
                'was found'
            )
